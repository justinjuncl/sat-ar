...
satellite.gstimeFromTime = function (time) {
    var julian_day = jday(time.getUTCFullYear(), time.getUTCMonth() + 1, time.getUTCDate(), time.getUTCHours(), time.getUTCMinutes(), time.getUTCSeconds());
    return gstime (julian_day);
}
...
satellite.propagate = function (satrec, time) {
    return propagate (satrec, time.getUTCFullYear(), time.getUTCMonth() + 1, time.getUTCDate(), time.getUTCHours(), time.getUTCMinutes(), time.getUTCSeconds());
}
...
function eciToGeodetic (eci_coords, gmst) {
    'use strict';
...
function eciToEcf (eci_coords, gmst){
    'use strict';
...
function ecfToEci (ecf_coords, gmst){
    'use strict';
    // ccar.colorado.edu/ASEN5070/handouts/coordsys.doc
    //
    // [X]     [C -S  0][X]
...
function geodeticToEcf (geodetic_coords){
    'use strict';
...
function topocentric (observer_coords, satellite_coords){
    ...
    var observer_ecf = geodeticToEcf (observer_coords);
    ...
    return { topS : top_s, topE : top_e, topZ : top_z };
}

function topocentricToLookAngles (topocentric){
    'use strict';
    var top_s = topocentric.topS;
    var top_e = topocentric.topE;
    var top_z = topocentric.topZ;
    ...
    return { azimuth : Az, elevation : El, rangeSat : range_sat };
}
...
satellite.eciToGeodetic = function (eci_coords, gmst) {
    return eciToGeodetic (eci_coords, gmst);
}
...
function dopplerFactor (my_location, position, velocity) {
...
satellite.pi = pi;
satellite.twopi = twopi;
satellite.deg2rad = deg2rad;
satellite.rad2deg = rad2deg;
satellite.minutesPerDay = minutesPerDay;
/*
 * satellite-js v1.1
 * (c) 2013 Shashwat Kandadai and UCSC
 * https://github.com/shashwatak/satellite-js
